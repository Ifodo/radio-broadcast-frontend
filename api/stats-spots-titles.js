// Vercel Serverless Function: Proxy to RBS /stats/spots/titles to avoid CORS
...
module.exports = async (req, res) => {
	...
	const { filename, limit, title } = req.query || {};
	const qs = new URLSearchParams();
	if (filename) qs.set('filename', String(filename));
	if (limit) qs.set('limit', String(limit));
	if (title) qs.set('title', String(title));
	const upstream = `https://rbs.elektranbroadcast.com/stats/spots/titles?${qs.toString()}`;
